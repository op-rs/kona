...
use crate::{StatelessL2Builder, TrieDBProvider, constants::FEE_RECIPIENT};
...
impl ExecutorTestFixtureCreator {
    ...
    pub(crate) async fn create_static_fixture(self) {
        ...
        let (block_header, _) =
            executor.build_block(payload_attrs).expect("Failed to execute block");
...
pub(crate) async fn run_test_fixture(fixture_path: PathBuf) {
    ...
    let (block_header, _) = executor.build_block(fixture.executing_payload).unwrap();
