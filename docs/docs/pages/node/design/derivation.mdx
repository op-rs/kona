...
The derivation providers include several security mechanisms that should be understood when configuring the system:

- **RPC Trust Mode**: Providers support a `trust_rpc` parameter that controls data verification
  - `trust_rpc: true` (default): Skips hash verification for better performance
  - `trust_rpc: false`: Verifies block and header hashes against expected values
- **Hash Verification**: When `trust_rpc: false`, providers verify that:
  - Block hashes match the requested hash (see [`AlloyL2ChainProvider::verify_block_hash`][l2_verify] in `crates/providers/providers-alloy/src/l2_chain_provider.rs`)
  - Header hashes are correctly computed (see [`AlloyChainProvider::verify_header_hash`][l1_verify] in `crates/providers/providers-alloy/src/chain_provider.rs`)
  - Verification occurs in `header_by_hash`, `block_info_and_transactions_by_hash` (L1), and `block_info_by_id` (L2) methods before caching
- **JWT Authentication**: L2 providers use JWT secrets for secure communication with execution engines
- **Cache Security**: LRU caches store blockchain data without re-verification when serving cached entries. This means:
  - Data cached with `trust_rpc: true` will be served from cache even if `trust_rpc: false` is set later
  - Cache entries bypass hash verification checks that would normally occur on fresh RPC calls
  - Consider cache poisoning risks if the RPC endpoint is untrusted or if `trust_rpc` settings change during runtime

**Production Recommendations**:
- Use `trust_rpc: false` for critical production environments
- Monitor for hash verification failures as potential attack indicators
- Ensure JWT secrets are properly managed and rotated
- Consider cache size limits based on security requirements
...
[l1_verify]: https://github.com/op-rs/kona/blob/main/crates/providers/providers-alloy/src/chain_provider.rs#L82-L105
[l2_verify]: https://github.com/op-rs/kona/blob/main/crates/providers/providers-alloy/src/l2_chain_provider.rs#L82-L99
