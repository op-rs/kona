...
```rust
use kona_node_service::{NodeActor, CancellableContext};
...
/// The context passed to the actor when starting.
struct MyCustomContext {
    cancellation: CancellationToken,
    // Add channels and other communication primitives here
}

impl CancellableContext for MyCustomContext {
...
impl NodeActor for MyCustomActor {
    ...
    type StartData = MyCustomContext;
    ...
    async fn start(self, ctx: Self::StartData) -> Result<(), Self::Error> {
        loop {
            tokio::select! {
                ...
                // Handle other events, messages, etc.
            }
        }
    }
}
...
The node is built using the `RollupNodeBuilder`, which constructs a
`RollupNode` with the configured actors and services. While the builder
provides sensible defaults, you can customize the node by:

1. Building your own binary that uses the `RollupNode` directly
2. Implementing custom actors that communicate via channels
3. Composing the built-in actors with your custom logic

**Example: Using RollupNodeBuilder**

```rust
use kona_node_service::{RollupNodeBuilder, L1ConfigBuilder, EngineConfig, NetworkConfig};
...
To add custom actors, you would typically spawn them alongside the
built-in actors and communicate via channels (e.g., `tokio::sync::mpsc`).
...
- Most users will want to start by using the standard `RollupNodeBuilder`
  and adding custom actors alongside the built-in ones.
