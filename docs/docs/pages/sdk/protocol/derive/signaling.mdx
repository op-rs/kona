...
Understanding signals first requires a more in-depth review of the result
...
pipeline returns a [`StepResult`][step-result]. Step results provide an
extensible way for pipeline stages to signal different results to the
...
The pipeline driver may choose to consume the payload attributes how it
...
grouping the inner error based on severity (or how they should be handled).
