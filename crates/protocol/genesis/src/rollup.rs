...
impl OpHardforks for RollupConfig {
    fn op_fork_activation(&self, fork: OpHardfork) -> ForkCondition {
        match fork {
            OpHardfork::Bedrock => ForkCondition::Timestamp(0),
            OpHardfork::Regolith => {
                ForkCondition::Timestamp(self.hardforks.regolith_time.unwrap_or_default())
            }
            OpHardfork::Canyon => {
                ForkCondition::Timestamp(self.hardforks.canyon_time.unwrap_or_default())
            }
            OpHardfork::Ecotone => {
                ForkCondition::Timestamp(self.hardforks.ecotone_time.unwrap_or_default())
            }
            OpHardfork::Fjord => {
                ForkCondition::Timestamp(self.hardforks.fjord_time.unwrap_or_default())
            }
            OpHardfork::Granite => {
                ForkCondition::Timestamp(self.hardforks.granite_time.unwrap_or_default())
            }
            OpHardfork::Holocene => {
                ForkCondition::Timestamp(self.hardforks.holocene_time.unwrap_or_default())
            }
            OpHardfork::Isthmus => {
                ForkCondition::Timestamp(self.hardforks.isthmus_time.unwrap_or_default())
            }
            OpHardfork::Interop => {
                ForkCondition::Timestamp(self.hardforks.interop_time.unwrap_or_default())
            }
        }
    }
}
