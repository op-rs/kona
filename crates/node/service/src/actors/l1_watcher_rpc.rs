...
impl NodeActor for L1WatcherRpc {
    ...
    async fn start(mut self) -> Result<(), Self::Error> {
        let mut head_stream =
            BlockStream::new(&self.l1_provider, BlockNumberOrTag::Latest, 13).into_stream();
        let mut finalized_stream =
            BlockStream::new(&self.l1_provider, BlockNumberOrTag::Finalized, 60).into_stream();
...
impl<'a> BlockStream<'a> {
    ...
    fn new(l1_provider: &'a RootProvider, tag: BlockNumberOrTag, poll_interval_secs: u64) -> Self {
        ...
        Self { l1_provider, tag, poll_interval_secs }
    }
