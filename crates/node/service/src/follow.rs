...
impl HttpFollowClient {
    /// Creates a new [`HttpFollowClient`] from the given configuration.
    ///
    /// # Arguments
    ///
    ...
    /// cannot be built.
    pub fn new(l2_url: Url, l1_url: Url) -> Result<Self, FollowClientError> {
        Self::new_with_timeout(l2_url, l1_url, Duration::from_millis(DEFAULT_FOLLOW_TIMEOUT))
    }

    /// Creates a new [`HttpFollowClient`] with a custom timeout.
    ///
    /// # Arguments
    ///
    /// * `l2_url` - The L2 consensus layer RPC endpoint URL
