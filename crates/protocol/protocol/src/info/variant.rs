...
impl L1BlockInfoTx {
    ...
    pub fn try_new(
        ...
        let blob_fee_config = if let Some(blob_fee_param) =
            blob_fee_params.active_scheduled_params_at_timestamp(l1_header.timestamp)
        {
            *blob_fee_param
        } else if l1_config.osaka_time.is_some_and(|time| time <= l1_header.timestamp) {
            BlobParams::osaka()
        } else if l1_config
