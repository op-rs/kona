...
The entry-point for the `kona-node` is the [`RollupNode`][node] struct,
constructed via the [`RollupNodeBuilder`][builder]. The `RollupNode`
encapsulates the core wiring for the node, connecting all the different
components and running each in a spawned thread. As such, each node
component is considered an actor.

Individual actors implement the [`NodeActor` trait][actor], which
defines how the actor is started and what data it receives.

Kona provides implementations for all `NodeActor`s required
to run a `RollupNode`. Actors are defined in the
...
The `RollupNode` spawns and orchestrates the following actors.
These are subject to change, but are currently defined as follows.
...
[node]: https://github.com/op-rs/kona/blob/main/crates/node/service/src/service/node.rs
[builder]: https://github.com/op-rs/kona/blob/main/crates/node/service/src/service/builder.rs
[actors]: https://github.com/op-rs/kona/tree/main/crates/node/service/src/actors
[actor]: https://github.com/op-rs/kona/blob/main/crates/node/service/src/actors/traits.rs
