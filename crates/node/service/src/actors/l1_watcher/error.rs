...
/// The error type for the `L1WatcherActorBuilder`.
#[derive(Error, Debug)]
pub enum L1WatcherActorBuilderError {
    /// Any error that can occur when calling the `build` method.
    #[error("L1WatcherActorBuilder: build error")]
    BuildError(String),
}
