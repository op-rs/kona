...
The sequencer functionality is built around the `SequencerActor`, which implements the `NodeActor` trait:

#### `NodeActor`
The base trait for all actors in the node:
...
#### `SequencerActor`
The core actor responsible for block production. It is a generic struct parameterized by several trait bounds:
