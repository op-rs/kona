...
pub struct HealthzRpc {
    ...
    pub rollup_boost_health: Option<mpsc::Sender<RollupBoostHealthQuery>>,
}

impl HealthzRpc {
    /// Constructs a new [`HealthzRpc`] given the rollup boost health sender.
    pub const fn new(rollup_boost_health: Option<mpsc::Sender<RollupBoostHealthQuery>>) -> Self {
...
impl HealthzApiServer for HealthzRpc {
    ...
    async fn healthz_rollup_boost(&self) -> RpcResult<RollupBoostHealth> {
        let Some(ref rollup_boost_health) = self.rollup_boost_health else {
