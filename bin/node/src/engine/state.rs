...
impl EngineState {
    ...
    pub fn new(finalized: L2BlockRef) -> Self {
        Self {
    ...
    pub fn unsafe_head(&self) -> L2BlockRef {
        self.unsafe_head
    }

    /// Returns the current cross-verified unsafe head.
    pub fn cross_unsafe_head(&self) -> L2BlockRef {
        self.cross_unsafe_head
    }

    /// Returns the current pending safe head.
    pub fn pending_safe_head(&self) -> L2BlockRef {
        self.pending_safe_head
    }

    /// Returns the current local safe head.
    pub fn local_safe_head(&self) -> L2BlockRef {
        self.local_safe_head
    }

    /// Returns the current safe head.
    pub fn safe_head(&self) -> L2BlockRef {
        self.safe_head
    }

    /// Returns the current finalized head.
    pub fn finalized_head(&self) -> L2BlockRef {
        self.finalized_head
    }

    /// Returns the current backup unsafe head.
    pub fn backup_unsafe_head(&self) -> L2BlockRef {
        self.backup_unsafe_head
    }

    /// Set the unsafe head.
    pub fn set_unsafe_head(&mut self, unsafe_head: L2BlockRef) {
        self.unsafe_head = unsafe_head;
        self.forkchoice_update_needed = true;
    }

    /// Set the cross-verified unsafe head.
    pub fn set_cross_unsafe_head(&mut self, cross_unsafe_head: L2BlockRef) {
        self.cross_unsafe_head = cross_unsafe_head;
    }

    /// Set the pending safe head.
    pub fn set_pending_safe_head(&mut self, pending_safe_head: L2BlockRef) {
        self.pending_safe_head = pending_safe_head;
    }

    /// Set the local safe head.
    pub fn set_local_safe_head(&mut self, local_safe_head: L2BlockRef) {
        self.local_safe_head = local_safe_head;
    }

    /// Set the safe head.
    pub fn set_safe_head(&mut self, safe_head: L2BlockRef) {
        self.safe_head = safe_head;
        self.forkchoice_update_needed = true;
    }

    /// Set the finalized head.
    pub fn set_finalized_head(&mut self, finalized_head: L2BlockRef) {
        self.finalized_head = finalized_head;
        self.forkchoice_update_needed = true;
    }

    /// Set the backup unsafe head.
    pub fn set_backup_unsafe_head(&mut self, backup_unsafe_head: L2BlockRef, reorg: bool) {
